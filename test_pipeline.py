...
def create_transparent_overlay(name: str, duration: float) -> str:
    ...
    # Create a transparent video with text (using prores for alpha)
    cmd = [
        FFMPEG_PATH, "-y",
        ...
        "-c:v", "png",  # Use PNG for lossless with alpha
        "-c:a", "aac", "-b:a", "128k",
        output_path
    ]
    ...
    if result.returncode == 0 and Path(output_path).exists():
        ...
        if result.returncode == 0 and Path(output_path).exists():
            return output_path
        print(f"Failed to create overlay {name}: {result.stderr[:200]}")
        return None
...
def test_concatenation():
    ...
    if success and Path(output_path).exists():
        ...
        if info and 8 < info.duration < 10:
            print(f"✓ Concatenation successful - Duration: {info.duration:.1f}s")
            return True
        else:
            print(f"✗ Unexpected duration: {info.duration if info else 'unknown'}")
            return False
    else:
...
def test_crossfade():
    ...
    if success and Path(output_path).exists():
        ...
        print(f"✓ Crossfade successful - Duration: {info.duration:.1f}s")
        return True
    else:
